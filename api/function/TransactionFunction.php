...
class TransactionFunction
{
    ...
    public function InsertTransaction($user_id, $product_id, $address, $days, $times, $amount, $notes)
    {
        if ($this->CheckUserUnpaid($user_id)) {
            ...
            $keteranganWaktu = substr($keteranganWaktu, 0, strlen($keteranganWaktu) - 2);
            ...
            for ($i = 0; $i < 7; $i++) {
                if (is_array($pecahanhari)) {
                    $pecahanhari[$i] = \substr($days, $i, 1);
                    if ($pecahanhari[$i] == 1) {
                        $keteranganHari .= "Senin, ";
                    } else if ($pecahanhari[$i] == 2) {
                        $keteranganHari .= "Selasa, ";
                    } else if ($pecahanhari[$i] == 3) {
                        $keteranganHari .= "Rabu, ";
                    } else if ($pecahanhari[$i] == 4) {
                        $keteranganHari .= "Kamis, ";
                    } else if ($pecahanhari[$i] == 5) {
                        $keteranganHari .= "Jumat, ";
                    } else if ($pecahanhari[$i] == 6) {
                        $keteranganHari .= "Sabtu, ";
                    } else if ($pecahanhari[$i] == 7) {
                        $keteranganHari .= "Minggu, ";
                    }
                }
            }
                    ...
                    for ($j = 1; $j <= 3; $j++) {
                        ...
                        if ($booleanhari[$i] == 1 && $booleanwaktu[$j - 1] == 1) {
                            if ($amount != 0) {
                                ...
                                if ($sekali) {
                                    $tanggalPertama = $datePesanan;
                                    $sekali = false;
                                }
                                $stmt = $this->conn->prepare("INSERT INTO transactions(`invoice`, `product_id`, `user_id`, `address`, `date`, `notes`, `times`, `status`, `created_at`, `updated_at`) VALUES(?,?,?,?,?,?,?,?,?,?)");
                                $status = 1;
                                if ($stmt != FALSE) {
                                    $stmt->bind_param("ssssssssss", $invoice, $product_id, $user_id, $address, $datePesanan, $notes, $j, $status, $datenow, $datenow);
                                    $amount--;
                                    if ($stmt->execute()) {
                                        $stmt->close();
            ...
            //Get price from package table
            $stmt = $this->conn->prepare("SELECT * FROM `packages` WHERE `unique_id`=?");
            if ($stmt != false) {
                $stmt->bind_param("s", $product_id);
                if ($stmt->execute()) {
                    $packageResult = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
            ...
            $stmt = $this->conn->prepare("INSERT INTO `payment`(`invoice`, `proof_of_payment`, `total_payment`, `amount`,`first_date`, `days`, `times`, `created_at`, `updated_at`) VALUES (?,?,?,?,?,?,?,?,?)");
            if ($stmt != False) {
                $totalPrice = $price * $banyakPorsi;
                $stmt->bind_param("sssssssss", $invoice, $proof, $totalPrice, $banyakPorsi, $tanggalPertama, $keteranganHari, $keteranganWaktu, $datenow, $datenow);
                if ($stmt->execute()) {
                    $stmt->close();
                } else {
                    $response['error'] = true;
    ...
    public function DietMayo($user_id, $notes, $address)
    {
        if ($this->CheckUserUnpaid($user_id)) {
            ...
            $banyakPorsi = 26;
            $keteranganHari = "Senin, Selasa, Rabu, Kamis, Jumat, Sabtu, Minggu";
    ...
    public function DietKhusus($user_id, $product_id, $days, $address, $times, $notes, $activity, $sickness, $foodTypes, $diagnose)
    {
        if ($this->CheckUserUnpaid($user_id)) {
            ...
            if ($stmt != false) {
                ...
                $response['error'] = true;
                $response['message'] = "Price can't be get";
                echo json_encode($response);
                return false;
            }
    ...
    {
        $stmt = $this->conn->prepare("UPDATE `transactions` SET `status`=? WHERE `invoice`=? ");
        $status = 2;
        if ($stmt != FALSE) {
            $stmt->bind_param("ss", $status, $invoice);
            if ($stmt->execute()) {
                $stmt->close();
                $stmt = $this->conn->prepare("UPDATE `payment` SET `proof_of_payment`=? WHERE `invoice`=?");
                if ($stmt != false) {
                    $stmt->bind_param("ss", $proof, $invoice);
                    if ($stmt->execute()) {
                        $stmt->close();
                        $stmt = $this->conn->prepare("SELECT * FROM transactions WHERE invoice=?");
                        $stmt->bind_param("s", $invoice);
    ...
    public function Delete($invoice)
    {
        ...
        if ($stmt != false) {
            ...
            $stmt->execute();
            $stmt->close();
            $stmt = $this->conn->prepare("DELETE FROM `payment` WHERE `invoice`=?");
            if ($stmt != false) {
                $stmt->bind_param("s", $invoice);
                if ($stmt->execute()) {
                    $stmt->close();
